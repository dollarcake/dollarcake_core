...
describe("staking contract", function() {
  ...
  it("should reward the contract properly", async function() {
    ...
    const stakerFee = 900 / 1000;
    const contentFee = 950 / 1000;

    await staking.changeStakerFee(900);
    await staking.changeContentCreatorFee(950);

    const amountToSend = ["1000000000000000000", "1000000000000000000"];
    const amountToSendContent = (Number(amountToSend[1]) / 2) * contentFee;
    const amountToSendStaker = (Number(amountToSend[1]) / 2) * stakerFee;
    const dollarCakeFee =
	amountToSend[1] - amountToSendStaker - amountToSendContent;
    await expect(staking.reward(receivers, amountToSend))
      .to.emit(staking, "Reward")
      .withArgs(
        receivers[1],
        amountToSend[1],
        amountToSendStaker.toString(),
        amountToSendContent.toString(),
        dollarCakeFee.toString()
      );
    ...
    assert.equal(
      ...
      amountToSendContent.toString(),
      "alice should have got half the payout"
    );
    assert.equal(
      aliceStake.toString(),
      amountToSendStaker.toString(),
      "contract should have got half the payout"
    );
    assert.equal(
    ...
    assert.equal(
      ...
      amountToSendStaker.toString() * 2,
      "contract should have got half the payout"
    );
    assert.equal(
      cakeAddressBalance.toString(),
      calculatedCakeAddressBalance.toString(),
      "cake address should have got rewarded"
    );
  });
  ...
  it("should reward the contract properly on 75/25 split", async function() {
    ...
    await expect(staking.connect(alice).setSplit(25))
      .to.emit(staking, "SplitUpdated")
      .withArgs(alice.address, "25");
    ...
    assert.equal(
      balanceOfAlice.toString(),
      calculatedAmountAlice.toString(),
      "alice should have got half the payout"
    );
    assert.equal(
      balanceOfContract.toString(),
      calculatedAmountContract.toString(),
      "contract should have got half the payout"
    );
  });
  ...
  it("should reward staking pool only", async function() {
    ...
    await expect(staking.rewardStakingPoolOnly(receivers, amounts))
      .to.emit(staking, "Reward")
      .withArgs(receivers[1], amounts[1], amounts[1], "0", "0");
  ...
  it("should deposit and withdraw alice 75% bob 25%", async function() {
    ...
    await expect(
      staking.connect(alice).deposit(charlie.address, aliceDeposit.toString())
    )
      .to.emit(staking, "UserDeposit")
      .withArgs(
        alice.address,
        charlie.address,
    ...
    assert.equal(
      bobStake.toString(),
      bobDeposit.toString(),
      "bob stake should equal deposit"
    );
    ...
    assert.equal(
      balanceOfAliceBefore.toString(),
      "0",
      "alice should have no tokens"
    );
    assert.equal(
      balanceOfBobBefore.toString(),
      "0",
    ...
    await increaseTime(ethers);
    await expect(
      staking.connect(alice).withdraw(charlie.address, aliceDeposit.toString())
    )
      .to.emit(staking, "UserWithdrawal")
      .withArgs(
        alice.address,
        charlie.address,
        aliceDeposit.toString(),
    ...
    const aliceStakeAfter = await staking.userStake(
      charlie.address,
      alice.address
    );
    ...
    assert.equal(
      balanceOfAliceAfter,
      aliceDeposit,
      "alice should have original amount of tokens"
    );
  ...
  it("should deposit and withdraw alice 75% bob 25% while extra tokens added before bob deposits", async function() {
    ...
    assert.equal(
      ...
      aliceDeposit.toString(),
      "staking contract should have alice deposit"
    );
    assert.equal(aliceStake, aliceDeposit, "alice stake should equal deposit");
    ...
    const withdrawPayout = await staking.withdrawPayout(
      charlie.address,
      aliceStake
    );
    ...
    assert.equal(aliceStakeAfter.toString(), "0", "alice should have no stake");
    assert.equal(
      withdrawPayout.toString(),
      "34125000000000000000",
      "getter function should work properly"
    );
    assert.equal(
      balanceOfAliceAfter.toString(),
      "34125000000000000000",
      "alice should have original amount of tokens"
    );
